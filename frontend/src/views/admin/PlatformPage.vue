  ...
  <div>
    ...
    <div class="flex items-center justify-between mb-6">
      ...
      <div class="flex gap-2">
        <button class="btn-secondary btn-sm" :disabled="cleaningUp" @click="handleCleanup">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
      <div class="bg-bg-card border border-border rounded-xl p-4">
        <p class="text-[11px] uppercase tracking-wider text-text-muted font-medium mb-1">Total Users</p>
        <p class="text-2xl font-bold text-text-primary">{{ usersTotal }}</p>
      </div>
      <div class="bg-bg-card border border-border rounded-xl p-4">
        <p class="text-[11px] uppercase tracking-wider text-text-muted font-medium mb-1">Platform Admins</p>
        <p class="text-2xl font-bold text-warning">{{ users.filter(u => u.is_platform_admin).length }}</p>
      </div>
      <div class="bg-bg-card border border-border rounded-xl p-4">
        <p class="text-[11px] uppercase tracking-wider text-text-muted font-medium mb-1">Organizations</p>
        <p class="text-2xl font-bold text-text-primary">{{ orgsTotal }}</p>
      </div>
      <div class="bg-bg-card border border-border rounded-xl p-4">
        <p class="text-[11px] uppercase tracking-wider text-text-muted font-medium mb-1">Active Orgs</p>
    ...
    <section class="mb-8">
      <div class="flex items-center justify-between mb-3">
        ...
        <button class="btn-ghost btn-sm" @click="loadingUsers = true; loadUsers()">
          <RefreshCw :size="14" /> Refresh
        </button>
      </div>

      <div v-if="loadingUsers"><Skeleton variant="card" :count="3" /></div>
      <div v-else-if="users.length === 0" class="bg-bg-card border border-border rounded-xl">
        ...
        <table class="data-table w-full">
          ...
          <tbody>
            <tr v-for="u in users" :key="u.id">
              ...
              <td>
                <span :class="['badge-sm', u.status === 'active' ? 'badge-success' : 'badge-warning']">
                  {{ u.status }}
                </span>
              </td>
              <td>
                <span v-if="u.is_platform_admin" class="badge-sm badge-warning flex items-center gap-1 w-fit">
                  <Shield :size="12" /> Platform Admin
                </span>
                <span v-else class="badge-sm badge-default">User</span>
              </td>
              <td class="text-text-muted text-sm">{{ formatDate(u.last_login_at) }}</td>
              <td class="text-text-muted text-sm">{{ formatDate(u.created_at) }}</td>
            </tr>
          </tbody>
        </table>
      </div>
    ...
    <section>
      <div class="flex items-center justify-between mb-3">
        ...
        <button class="btn-ghost btn-sm" @click="loadingOrgs = true; loadOrgs()">
          <RefreshCw :size="14" /> Refresh
        </button>
      </div>

      <div v-if="loadingOrgs"><Skeleton variant="card" :count="2" /></div>
      <div v-else-if="orgs.length === 0" class="bg-bg-card border border-border rounded-xl">
        ...
        <table class="data-table w-full">
          ...
          <tbody>
            <tr v-for="org in orgs" :key="org.id">
              <td class="font-medium text-text-primary">{{ org.name }}</td>
              <td class="text-text-muted text-sm font-mono">{{ org.slug }}</td>
              <td>
                <span :class="['badge-sm', org.status === 'active' ? 'badge-success' : 'badge-warning']">
                  {{ org.status }}
                </span>
              </td>
              <td class="text-text-muted text-sm">{{ org.memberCount ?? '-' }}</td>
              <td class="text-text-muted text-sm">{{ formatDate(org.created_at) }}</td>
              <td>
                <button class="btn-ghost btn-sm" @click="handleSwitchToOrg(org.id)">
                  Switch
                </button>
              </td>
            </tr>
