...
import { Mail, MousePointer, Eye, AlertTriangle, Clock, Tag, Zap, Link, Loader2, TrendingUp, BarChart3 } from 'lucide-vue-next'
    ...
    <div v-else-if="profile" class="space-y-5">
      ...
      <div class="flex items-start gap-4">
        ...
        <div class="flex-1 min-w-0">
          ...
          <div class="text-sm text-text-muted">{{ profile.contact.email }}</div>
          <div v-if="profile.contact.company" class="text-xs text-text-muted mt-0.5">{{ profile.contact.company }}</div>
        </div>
        <div class="text-right shrink-0">
          ...
          <div class="text-[10px] text-text-muted">Engagement</div>
        </div>
      </div>
      ...
      <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div class="bg-surface-0 rounded-lg p-3 text-center">
          <Mail :size="14" class="mx-auto text-text-muted mb-1" />
          <div class="text-lg font-bold text-text-primary">{{ profile.stats.emails_sent }}</div>
          <div class="text-[10px] text-text-muted">Emails Sent</div>
        </div>
        <div class="bg-surface-0 rounded-lg p-3 text-center">
          <Eye :size="14" class="mx-auto text-green-400 mb-1" />
          <div class="text-lg font-bold text-text-primary">{{ profile.stats.open_rate }}%</div>
          <div class="text-[10px] text-text-muted">Open Rate</div>
        </div>
        <div class="bg-surface-0 rounded-lg p-3 text-center">
          <MousePointer :size="14" class="mx-auto text-blue-400 mb-1" />
          <div class="text-lg font-bold text-text-primary">{{ profile.stats.click_rate }}%</div>
          <div class="text-[10px] text-text-muted">Click Rate</div>
        </div>
        <div class="bg-surface-0 rounded-lg p-3 text-center">
          <BarChart3 :size="14" class="mx-auto text-violet-400 mb-1" />
          <div class="text-lg font-bold text-text-primary">{{ profile.stats.campaigns_engaged }}</div>
          <div class="text-[10px] text-text-muted">Campaigns</div>
        </div>
      </div>

      <!-- Open Time Distribution -->
      <div v-if="openHours.length > 0">
        <div class="text-xs font-semibold text-text-secondary mb-2">Best Open Times</div>
        <div class="flex items-end gap-px h-16 bg-surface-0 rounded-lg p-2">
          <div
        ...
        <div class="flex justify-between text-[9px] text-text-muted mt-1 px-2">
      ...
      <div v-if="profile.top_links?.length > 0">
        <div class="text-xs font-semibold text-text-secondary mb-2">Top Clicked Links</div>
        <div class="space-y-1.5">
          <div v-for="link in profile.top_links.slice(0, 5)" :key="link.url" class="flex items-center gap-2 text-xs">
            <Link :size="11" class="text-text-muted shrink-0" />
            <a :href="link.url" target="_blank" class="text-text-secondary hover:text-accent truncate flex-1">{{ link.url }}</a>
            <span class="text-text-muted shrink-0">{{ link.clicks }}x</span>
          </div>
        </div>
      </div>
      ...
      <div v-if="profile.recent_events?.length > 0">
        <div class="text-xs font-semibold text-text-secondary mb-2">Recent Activity</div>
        <div class="space-y-1">
          <div v-for="(event, i) in profile.recent_events.slice(0, 8)" :key="i" class="flex items-center gap-2 text-xs py-1.5 border-b border-border last:border-0">
            <component :is="eventIcons[event.type] || Clock" :size="11" class="text-text-muted shrink-0" />
            <span class="text-text-secondary flex-1 truncate">{{ event.type.replace(/_/g, ' ') }}</span>
            <span class="text-[10px] text-text-muted shrink-0">{{ formatDate(event.created_at) }}</span>
          </div>
        </div>
      </div>
      ...
      <div v-if="profile.contact.tags">
        <div class="text-xs font-semibold text-text-secondary mb-2">Tags</div>
        <div class="flex flex-wrap gap-1">
          <span
            v-for="tag in (typeof profile.contact.tags === 'string' ? JSON.parse(profile.contact.tags || '[]') : profile.contact.tags)"
            :key="tag"
            class="px-2 py-0.5 bg-surface-0 border border-border rounded text-[11px] text-text-secondary"
          >{{ tag }}</span>
        </div>
      </div>
    </div>
