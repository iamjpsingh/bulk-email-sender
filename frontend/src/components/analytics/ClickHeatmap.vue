  ...
  <div class="bg-bg-card border border-border rounded-xl overflow-hidden">
    <div class="px-5 py-4 border-b border-border flex items-center justify-between">
      <div class="flex items-center gap-2">
        ...
        <h3 class="text-sm font-semibold text-text-primary">Click Heatmap</h3>
        <span v-if="!loading" class="text-xs text-text-muted">({{ totalClicks.toLocaleString() }} total clicks)</span>
      </div>
    </div>
    ...
    <div v-else-if="links.length === 0" class="px-5 py-8 text-center text-sm text-text-muted">No click data available</div>
    ...
    <div v-else class="p-5">
        ...
        <div v-for="link in links" :key="link.url" class="group">
          <div class="flex items-center justify-between gap-3 mb-1">
            <a
              ...
              class="text-xs text-text-secondary hover:text-accent transition truncate flex-1 flex items-center gap-1"
              :title="link.url"
            >
              {{ shortenUrl(link.url) }}
            ...
            <div class="flex items-center gap-3 shrink-0 text-xs">
              <span class="text-text-muted">{{ link.unique_clicks }} unique</span>
              <span class="font-semibold text-text-primary w-12 text-right">{{ link.click_count }}</span>
            </div>
          </div>
          <div class="h-6 bg-surface-0 rounded-md overflow-hidden relative">
            <div
      ...
      <div class="flex items-center gap-3 mt-5 pt-4 border-t border-border justify-center">
        <span class="text-[10px] text-text-muted">Cool</span>
        ...
        <span class="text-[10px] text-text-muted">Hot</span>
      </div>
    </div>
  </div>
</template>
