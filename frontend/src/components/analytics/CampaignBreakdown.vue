  ...
  <div class="bg-bg-card border border-border rounded-xl overflow-hidden">
    ...
    <div class="flex border-b border-border">
      <button
        ...
        :class="activeTab === tab.key ? 'border-accent text-accent' : 'border-transparent text-text-muted hover:text-text-secondary'"
        @click="activeTab = tab.key as any"
      >{{ tab.label }}</button>
    </div>

    <div v-if="loading" class="flex justify-center py-10"><Loader2 :size="18" class="animate-spin text-text-muted" /></div>
    ...
    <div v-else class="p-4">
      ...
      <div v-if="activeTab === 'devices'" class="space-y-4">
        <!-- Device types -->
        <div>
          <div class="text-[10px] font-semibold text-text-muted uppercase tracking-wider mb-2">Device Type</div>
          <div class="flex gap-3">
            <div v-for="d in devices" :key="d.name" class="flex-1 bg-surface-0 rounded-lg p-3 text-center">
              <component :is="deviceIcons[d.name] || Monitor" :size="18" class="mx-auto text-text-muted mb-1" />
              <div class="text-sm font-bold text-text-primary">{{ d.percentage }}%</div>
              <div class="text-[10px] text-text-muted capitalize">{{ d.name }} ({{ d.count }})</div>
            </div>
          </div>
          <div v-if="devices.length === 0" class="text-xs text-text-muted text-center py-4">No device data yet</div>
        </div>

        <!-- Browsers -->
        <div>
          <div class="text-[10px] font-semibold text-text-muted uppercase tracking-wider mb-2">Browsers</div>
          <div class="space-y-1.5">
            <div v-for="(b, i) in browsers.slice(0, 6)" :key="b.name" class="flex items-center gap-2">
              <span class="w-20 text-xs text-text-secondary truncate">{{ b.name }}</span>
              <div class="flex-1 h-4 bg-surface-0 rounded overflow-hidden">
                <div class="h-full rounded" :style="{ width: b.percentage + '%', backgroundColor: barColor(i) }"></div>
              </div>
              <span class="w-12 text-xs text-text-muted text-right">{{ b.percentage }}%</span>
            </div>
          </div>
        </div>
        ...
        <!-- OS -->
        <div>
          <div class="text-[10px] font-semibold text-text-muted uppercase tracking-wider mb-2">Operating Systems</div>
          <div class="space-y-1.5">
            <div v-for="(o, i) in oses.slice(0, 6)" :key="o.name" class="flex items-center gap-2">
              <span class="w-20 text-xs text-text-secondary truncate">{{ o.name }}</span>
              <div class="flex-1 h-4 bg-surface-0 rounded overflow-hidden">
                <div class="h-full rounded" :style="{ width: o.percentage + '%', backgroundColor: barColor(i + 3) }"></div>
              </div>
              <span class="w-12 text-xs text-text-muted text-right">{{ o.percentage }}%</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div v-if="activeTab === 'clients'">
        <div class="space-y-2">
          <div v-for="(cl, i) in clients" :key="cl.name" class="flex items-center gap-2">
            <Mail :size="12" class="text-text-muted shrink-0" />
            <span class="w-28 text-xs text-text-secondary truncate">{{ cl.name }}</span>
            <div class="flex-1 h-5 bg-surface-0 rounded overflow-hidden">
            ...
            <span class="w-10 text-xs text-text-muted text-right">{{ cl.count }}</span>
          </div>
          <div v-if="clients.length === 0" class="text-xs text-text-muted text-center py-6">No email client data yet. Deploy tracking Worker for detailed analytics.</div>
        </div>
      </div>
      ...
      <div v-if="activeTab === 'sources'">
        <div class="space-y-2">
          <div v-for="(s, i) in sources" :key="s.source" class="flex items-center gap-2">
            ...
            <Globe v-else :size="12" class="text-text-muted shrink-0" />
            <span class="w-28 text-xs text-text-secondary truncate">{{ s.source }}</span>
            <div class="flex-1 h-5 bg-surface-0 rounded overflow-hidden">
              <div class="h-full rounded" :style="{ width: Math.max((s.count / Math.max(sources[0]?.count || 1, 1)) * 100, 3) + '%', backgroundColor: barColor(i) }"></div>
            </div>
            <span class="w-10 text-xs text-text-muted text-right">{{ s.count }}</span>
          </div>
          <div v-if="sources.length === 0" class="text-xs text-text-muted text-center py-6">No referral data yet. Click tracking captures referral sources.</div>
        </div>
      </div>
    </div>
  </div>
