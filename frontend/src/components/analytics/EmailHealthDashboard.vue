...
import { Heart, TrendingUp, TrendingDown, AlertTriangle, CheckCircle, Info, Loader2, ShieldCheck, ShieldAlert, ShieldX } from 'lucide-vue-next'
...
const scoreColor = computed(() => {
  if (!health.value) return 'text-text-muted'
...
const scoreBg = computed(() => {
  if (!health.value) return 'bg-surface-1'
...
const ratingBadge = computed(() => {
  if (!health.value) return { text: 'Unknown', class: 'bg-surface-2 text-text-muted' }
  switch (health.value.rating) {
    ...
    default: return { text: health.value.rating, class: 'bg-surface-2 text-text-muted' }
  }
})
  ...
  <div class="bg-bg-card border border-border rounded-xl overflow-hidden">
    <div class="px-5 py-4 border-b border-border flex items-center justify-between">
      <div class="flex items-center gap-2">
        ...
        <h3 class="text-sm font-semibold text-text-primary">Email Health Score</h3>
      </div>
      <button v-if="!loading" @click="loadHealth" class="text-xs text-text-muted hover:text-text-secondary transition">Refresh</button>
    </div>
    ...
    <div v-else-if="error" class="px-5 py-8 text-center text-sm text-text-muted">{{ error }}</div>
    ...
    <div v-else-if="health" class="p-5">
      ...
      <div class="flex items-center gap-5 mb-6">
        ...
        <div>
          ...
          <p class="text-xs text-text-muted mt-2">Based on {{ health.metrics.total_sent.toLocaleString() }} emails sent</p>
        </div>
      </div>
      ...
      <div class="grid grid-cols-2 sm:grid-cols-5 gap-3 mb-5">
        <div :class="[statusBgs[metricStatus(health.metrics.bounce_rate, 2, 5)], 'rounded-lg p-3 text-center']">
          ...
          <div class="text-[10px] text-text-muted mt-0.5">Bounce Rate</div>
          <div class="text-[9px] text-text-muted">target: &lt;2%</div>
        </div>
        <div :class="[statusBgs[metricStatus(health.metrics.complaint_rate, 0.1, 0.5)], 'rounded-lg p-3 text-center']">
          ...
          <div class="text-[10px] text-text-muted mt-0.5">Complaint Rate</div>
          <div class="text-[9px] text-text-muted">target: &lt;0.1%</div>
        </div>
        <div :class="[statusBgs[metricStatus(health.metrics.unsubscribe_rate, 0.5, 2)], 'rounded-lg p-3 text-center']">
          ...
          <div class="text-[10px] text-text-muted mt-0.5">Unsub Rate</div>
          <div class="text-[9px] text-text-muted">target: &lt;0.5%</div>
        </div>
        <div :class="[statusBgs[engagementStatus(health.metrics.open_rate, 20, 10)], 'rounded-lg p-3 text-center']">
          ...
          <div class="text-[10px] text-text-muted mt-0.5">Open Rate</div>
          <div class="text-[9px] text-text-muted">benchmark: &gt;20%</div>
        </div>
        <div :class="[statusBgs[engagementStatus(health.metrics.click_rate, 3, 1)], 'rounded-lg p-3 text-center']">
          ...
          <div class="text-[10px] text-text-muted mt-0.5">Click Rate</div>
          <div class="text-[9px] text-text-muted">benchmark: &gt;3%</div>
        </div>
      </div>

      <!-- Recommendations -->
      <div v-if="health.recommendations.length > 0" class="space-y-2">
        <div class="text-xs font-semibold text-text-secondary mb-1">Recommendations</div>
        <div
          v-for="(rec, i) in health.recommendations" :key="i"
          class="flex items-start gap-2 text-xs text-text-muted bg-surface-0 rounded-lg px-3 py-2"
        >
