...
const statusConfig = computed(() => {
  ...
  const campaignMap: Record<string, { class: string; label: string }> = {
    draft: { class: 'bg-[rgba(148,163,184,0.15)] text-text-muted', label: 'Draft' },
    ...
    archived: { class: 'bg-[rgba(148,163,184,0.15)] text-text-muted', label: 'Archived' },
  }
  ...
  return map[s] || { class: 'bg-[rgba(148,163,184,0.15)] text-text-muted', label: props.status }
})
</script>
